"""
REPLAY SINIFLANDIRICILARI - TEMİZ DDPM/S2S İLE
================================================
4 yeni Replay sınıflandırıcısı eğitir. GTX 1080 (GPU 1) kullanılır.

KONFİGÜRASYONLAR:
  1. Replay + 500 temiz DDPM           → eski "ddpm 2K kirli" (0.606) ile karşılaştır
  2. Replay + 2K temiz DDPM            → eski "ddpm 2K kirli" (0.606) ile karşılaştır
  3. Replay + 5K temiz DDPM            → doz çalışması (yeni)
  4. Replay + 2K temiz DDPM + 500 S2S  → eski "ddpm+s2s 500 kirli" (0.659) ile karşılaştır
                                          *EN KRİTİK KARŞILAŞTIRMA*

ÇIKTI:
  Modeller: ~/Desktop/fpad_diffusion/results/yepyeni_resnet/
    - resnet18_replay_clean_ddpm_500.pth
    - resnet18_replay_clean_ddpm_2000.pth
    - resnet18_replay_clean_ddpm_5000.pth
    - resnet18_replay_clean_ddpm_s2s.pth

  Log: ~/Desktop/fpad_diffusion/results/yepyeni_resnet/clean_replay_history.txt

ÇALIŞTIRMA:
    nohup python train_clean_replay_classifiers.py > train_clean.log 2>&1 &

SONRAKİ ADIM (training bittikten sonra):
    python eer_threshold_eval_yepyeni.py
    # Yeni modeller otomatik test edilir, yepyeni_comparison_summary.txt güncellenir
"""
...
# YENI: Temiz DDPM sentetik (önceki "kirli" değil!)
CLEAN_DDPM_REPLAY = f"{DATA_ROOT}/synthetic/ReplayAttack/DDPM_spoof_fixed/spoof"

# YENI: Temiz S2S sentetik
CLEAN_S2S_REPLAY = f"{DATA_ROOT}/S2S_fixed/ReplayAttack"
...
class FlexibleDataset(Dataset):
    """Real ve attack klasörlerini birlikte okur (Replay yapısı için)"""
...
def train_config(name, synth_sources, save_path):
    ...
    # CUDA_VISIBLE_DEVICES=1 olduğu için cuda:0 → GTX 1080

    # Orijinal veri
    orig_dataset = FlexibleDataset(REPLAY_TRAIN, transform=train_transform)
    real_count = orig_dataset.labels.count(1.0)
    attack_count = orig_dataset.labels.count(0.0)
    print(f"  Orijinal Replay: Real={real_count}, Attack={attack_count}")
    ...
    dev_ds = FlexibleDataset(REPLAY_DEV, transform=test_transform)
...
def main():
    ...
    print(f"\n{'#'*75}")
    print(f"# TEMİZ REPLAY SINIFLANDIRICILARI")
    print(f"# GPU: GTX 1080 (CUDA_VISIBLE_DEVICES=1)")
    ...
    configs = [
        {
            'name': '[1/5] REPLAY - Real + 500 TEMİZ DDPM',
            'synth_sources': [(CLEAN_DDPM_REPLAY, 500)],
            'save_path': os.path.join(OUTPUT_DIR, 'resnet18_replay_clean_ddpm_500.pth'),
        },
        {
            'name': '[2/5] REPLAY - Real + 2K TEMİZ DDPM',
            'synth_sources': [(CLEAN_DDPM_REPLAY, 2000)],
            'save_path': os.path.join(OUTPUT_DIR, 'resnet18_replay_clean_ddpm_2000.pth'),
        },
        {
            'name': '[3/5] REPLAY - Real + 5K TEMİZ DDPM',
            'synth_sources': [(CLEAN_DDPM_REPLAY, 5000)],
            'save_path': os.path.join(OUTPUT_DIR, 'resnet18_replay_clean_ddpm_5000.pth'),
        },
        {
            'name': '[4/5] REPLAY - Real + 10K TEMİZ DDPM',
            'synth_sources': [(CLEAN_DDPM_REPLAY, 10000)],
            'save_path': os.path.join(OUTPUT_DIR, 'resnet18_replay_clean_ddpm_10000.pth'),
        },
        {
            'name': '[5/5] REPLAY - Real + 2K TEMİZ DDPM + 500 TEMİZ S2S **EN KRİTİK**',
            'synth_sources': [
                (CLEAN_DDPM_REPLAY, 2000),
                (CLEAN_S2S_REPLAY, 500),
            ],
            'save_path': os.path.join(OUTPUT_DIR, 'resnet18_replay_clean_ddpm_s2s.pth'),
        },
    ]
    ...
    # ÖZET
    print(f"\n\n{'='*75}")
    print(f"TEMİZ REPLAY SONUÇ ÖZETİ")
    print(f"{'='*75}\n")

    log_path = os.path.join(OUTPUT_DIR, 'clean_replay_history.txt')
    with open(log_path, 'w') as f:
        f.write("FPAD - TEMİZ REPLAY SINIFLANDIRICILARI\n")
        f.write("=" * 75 + "\n\n")
    ...
    print(f"  (Tüm modeller test edilir, yepyeni_comparison_summary.txt güncellenir)")
    print(f"{'='*75}\n")

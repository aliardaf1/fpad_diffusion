...
# 3. EĞİTİM DÖNGÜSÜ
# ==========================================
def train_ultimate_augmented():
    # Titan V kartını kullanıyoruz
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print(f"ULTIMATE S2S Hibrit Eğitim Başlıyor... Cihaz: {device}")
    ...
    # 1. Sentetik Veri Yolu (DDPM 50K)
    DDPM_SPOOF_DIR = "/home/undergrad25_1/Desktop/fpad_diffusion/data/synthetic/ReplayAttack/DDPM_spoof/spoof"
    
    # 2. Sentetik Veri Yolu (Spoof-to-Spoof 5K)
    S2S_SPOOF_DIR = "/home/undergrad25_1/Desktop/fpad_diffusion/data/S2S/ReplayAttack"
    ...
    print("3 Farklı Veri Seti Potada Birleştiriliyor...")
    ...
    print(f"Orijinal Train: {len(orig_train_ds)} | DDPM Attack: {len(ddpm_spoof_ds)} | S2S Attack: {len(s2s_spoof_ds)}")
    ...
    for epoch in range(20):
        model.train()
        running_loss = 0.0
        train_bar = tqdm(train_loader, desc=f"Epoch {epoch+1}/20 [Ultimate Eğitim]", mininterval=10)
        ...
        # Nihai model ismi güncellendi
        if hter < best_hter:
            best_hter = hter
            torch.save(model.state_dict(), "resnet18_replay_ultimate_s2s_best.pth")
            print(">> Yeni en iyi ULTIMATE S2S modeli kaydedildi!")
        print("-" * 60)

if __name__ == "__main__":
    train_ultimate_augmented()

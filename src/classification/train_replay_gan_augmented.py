...
# 3. EĞİTİM DÖNGÜSÜ
# ==========================================
def train_replay_gan_augmented():
    device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
    print(f"Replay-Attack Hibrit (GAN) Eğitimi Başlıyor... Cihaz: {device}")
    ...
    # DİKKAT: OT-CycleGAN'in ReplayAttack sahtelerini kaydettiği yolu buraya yazın!
    SYNTH_GAN_ATTACK = "/home/undergrad25_1/Desktop/fpad_diffusion/data/gan_synthetic_replay" 
    ...
    print(f"Orijinal Replay Train: {len(orig_train_ds)} | GAN Attack: {len(synth_attack_ds)}")
    ...
    for epoch in range(20):
        model.train()
        running_loss = 0.0
        train_bar = tqdm(train_loader, desc=f"Epoch {epoch+1}/20 [GAN Hibrit]", mininterval=10)
        ...
        if hter < best_hter:
            best_hter = hter
            torch.save(model.state_dict(), "resnet18_replay_gan_augmented_best.pth")
            print(">> Yeni en iyi GAN Hibrit modeli kaydedildi!")
        print("-" * 60)

if __name__ == "__main__":
    train_replay_gan_augmented()
